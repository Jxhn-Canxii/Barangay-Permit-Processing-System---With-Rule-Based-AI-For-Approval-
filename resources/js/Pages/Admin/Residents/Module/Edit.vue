<template>
    <div>
        <button
            @click.prevent="editResidentBehavior()"
            :class="{ 'opacity-25': isEditResidentModal }"
            :disabled="isEditResidentModal"
            class="px-3 py-2 bg-yellow-500 text-nowrap font-bold mb-4 text-md float-end text-white rounded shadow"
        >
            <i class="fa fa-edit"></i> Edit
        </button>

        <Modal :show="isEditResidentModal" :maxWidth="'2xl'" title="Edit Resident" @close="isEditResidentModal = false; errors = {}">
            <div class="grid grid-cols-1 gap-6 p-6">
                 <form class="mt-4" @submit.prevent="editResident">
                    <!-- Census Information -->
                    <div class="mb-4 p-1">
                    ...
                    <div class="flex items-center justify-end mt-4">
                        <button type="submit" class="bg-lime-500 text-white font-bold py-2 px-4 rounded">
...
const isEditResidentModal = ref(false);
...
const form = useForm({
    ...
    number_of_house_members: 0,
    household_members: [],
});


const editResidentBehavior = () => {
    const data = props.data;

    form.id = data.id;
    form.date_of_census = data.dateofcencus;
    form.area_of_census_street = data.areaofcencusstreet;
    form.first_name = data.firstname;
    form.middle_name = data.middlename;
    form.last_name = data.lastname;
    form.birthday = data.birthday;
    form.age = data.age;
    form.gender = data.gender;
    form.civil_status = data.civilstatus;
    form.current_school_enrollment = data.currentschoolenrollment;
    form.educational_attainment = data.educationalattainment;
    form.employment_status = data.employmentstatus;
    form.occupation = data.occupation;
    form.house_number = data.housenumber;
    form.street_name = data.streetname;
    form.barangay = data.barangay;
    form.city = data.city;
    form.province = data.province;
    form.house_type = data.housetype;
    form.health_status = data.healthstatus;
    form.disability_status = data.disabilitystatus;
    form.existing_health_condition = data.existinghealthcondition;
    form.fully_immunized = data.fullyimmunized;
    form.covid_19_vaccination = data.covid19vaccination;
    form.housing_type = data.housingtype;
    form.year_of_constructed = data.yearofconstructed;
    form.resident_lived = data.residentlived;
    form.mobile_number = data.mobilenumber;
    form.emergency_contact_name = data.emergencycontactname;
    form.emergency_contact_number = data.emergencycontactnumber;
    form.relationship_to_emergency_contact = data.relationshiptoemergencycontact;
    form.number_of_house_members = data.numberofhousemembers;
    form.household_members = data.householdmembers;

    isEditResidentModal.value = true;
}


const editResident = async () => {
    try {
        await axios.patch(route("residents.update", { id: form.id }), form);
        Swal.fire("Success!", "Resident updated successfully.", "success");
        form.reset();
        isEditResidentModal.value = false;
